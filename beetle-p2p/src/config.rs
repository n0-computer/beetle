...
use beetle_util::{beetle_data_root, insert_into_config_map};
...
/// For example, `BEETLE_P2P_MDNS=true` would set the value of the `Libp2pConfig.mdns` field
pub const ENV_PREFIX: &str = "BEETLE_P2P";
...
/// another binary like in beetle-one, beetle-share or beetle-embed.
...
/// Configuration for the [`beetle-p2p`] node.
#[derive(PartialEq, Debug, Clone, Deserialize, Serialize)]
pub struct Config {
    /// Configuration for libp2p.
    pub libp2p: Libp2pConfig,
    /// Configuration of RPC to other beetle services.
...
impl Config {
    pub fn default_with_rpc(client_addr: P2pAddr) -> Self {
        Self {
            ...
            key_store_path: beetle_data_root().unwrap(),
        }
    }
