...
use beetle_util::{insert_into_config_map, make_config};
...
fn test_make_config() {
    ...
    temp_env::with_vars(
        vec![
            ...
            ("BEETLE_TEST__PORT", Some("4000")),
            // set metrics fiels using `env_prefix`, double-underbar for deep
            // nesting
            ("BEETLE_TEST__METRICS__FOO", Some("true")),
            // set metrics field using `BEETLE_METRICS` prefix
            ("BEETLE_METRICS__BAR", Some("10")),
            // custom metrics env var
            ("BEETLE_INSTANCE_ID", Some("new_id")),
            // custom metrics env var
            ("BEETLE_ENV", Some("new_service_env")),
        ],
        || {
            let got = make_config(
                TestConfig::new(),
