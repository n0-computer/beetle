...
/// Manages a lock file used to track if an beetle program is already running.
/// Aquired locks write a file to beetle's application data path containing the
...
impl ProgramLock {
    ...
    pub fn new(prog_name: &str) -> Result<Self> {
        let path = crate::beetle_data_path(&format!("{prog_name}.lock"))
    ...
    fn write(&mut self) -> Result<()> {
        ...
        std::fs::create_dir_all(crate::beetle_data_root()?)?;
