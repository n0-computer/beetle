...
use axum::Router;
use beetle_resolver::dns_resolver::Config as DnsResolverConfig;
use beetle_rpc_types::gateway::GatewayAddr;
use beetle_unixfs::content_loader::ContentLoader;
...
mod tests {
    ...
    use beetle_unixfs::builder::{DirectoryBuilder, FileBuilder};
    use beetle_unixfs::content_loader::{FullLoader, FullLoaderConfig};
    use beetle_unixfs::unixfs::UnixfsNode;
    use cid::Cid;
    ...
    async fn spawn_store() -> (StoreAddr, tokio::task::JoinHandle<()>) {
        ...
        let config = beetle_store::Config::new(store_dir.path().join("db"));
        let store = beetle_store::Store::create(config).await.unwrap();
        let task =
            tokio::spawn(async move { beetle_store::rpc::new(server_addr, store).await.unwrap() });
        (client_addr, task)
    }
    ...
    async fn test_raw_fetch() {
        ...
        assert_eq!(http::StatusCode::OK, res.status());
        let body = hyper::body::to_bytes(res.into_body()).await.unwrap();
        let ufs = beetle_unixfs::unixfs::UnixfsNode::decode(&test_setup.root_cid, body).unwrap();
        assert_eq!(ufs.typ().unwrap(), beetle_unixfs::unixfs::DataType::File);
    }
    ...
    async fn test_fetch_car_recursive() {
        ...
        let car_reader = beetle_car::CarReader::new(body).await.unwrap();
