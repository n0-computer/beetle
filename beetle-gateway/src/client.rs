...
use anyhow::Result;
use beetle_car::{CarHeader, CarWriter};
use beetle_metrics::{
...
use beetle_resolver::dns_resolver::Config;
use beetle_resolver::resolver::{CidOrDomain, Metadata, Out, OutPrettyReader, OutType, Resolver};
use beetle_unixfs::{content_loader::ContentLoader, Source};
use bytes::Bytes;
use cid::Cid;
...
impl<T: ContentLoader + std::marker::Unpin> Client<T> {
    ...
    pub async fn retrieve_path_metadata(
        &self,
        path: beetle_resolver::resolver::Path,
...
pub struct IpfsRequest {
    ...
    pub cid: CidOrDomain,
    pub resolved_path: beetle_resolver::resolver::Path,
