...
use beetle_gateway::{bad_bits::BadBits, core::Core, metrics};
...
async fn main() -> Result<()> {
    let mut lock = ProgramLock::new("beetle-one")?;
    ...
    let (store_rpc, p2p_rpc) = {
        ...
        let store_rpc = beetle_one::mem_store::start(store_recv, config.store.clone()).await?;

        let p2p_rpc = beetle_one::mem_p2p::start(p2p_recv, config.p2p.clone()).await?;
        (store_rpc, p2p_rpc)
    };
