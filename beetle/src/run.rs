...
use beetle_util::{beetle_config_path, human, make_config};
...
#[clap(about = "A next generation IPFS implementation: https://beetle.computer")]
#[clap(after_help = doc::BEETLE_LONG_DESCRIPTION)]
pub struct Cli {
    #[clap(long)]
    cfg: Option<PathBuf>,
...
enum Commands {
    P2p(P2p),
    #[clap(about = "Add a file or directory to beetle & make it available on IPFS")]
    ...
    #[clap(about = "Start local beetle services")]
    ...
    #[clap(about = "Check the health of the different beetle services")]
    ...
    #[clap(about = "Stop local beetle services")]
...
impl Cli {
    pub async fn run(&self) -> Result<()> {
        ...
        let metrics_handler = beetle_metrics::MetricsHandle::new(MetricsConfig::default())
            .await
        ...
        let api = beetle_api::Api::from_env(self.cfg.as_deref(), self.make_overrides_map()).await?;
    ...
    async fn cli_command(&self, config: &Config, api: &Api) -> Result<()> {
        match &self.command {
            ...
            Commands::Get {
                ...
                let root_path =
                    beetle_api::fs::write_get_stream(path, blocks, output.as_deref()).await?;
...
async fn add(
    ...
    match (provide, svc_status.p2p.status()) {
        (true, StatusType::Down) => {
            ...
            "hint: try using the --offline flag, or run 'beetle start p2p'".yellow()
            )
        }
