...
pub const BEETLE_STORE: &str = "beetle-store";

// ToDo: Remove this function
// Related issue: https://github.com/n0-computer/beetle/issues/593
...
mod tests {
    ...
    fn test_verify_hash() {
        for codec in [IpldCodec::DagCbor, IpldCodec::DagJson] {
            ...
            assert_eq!(beetle_util::verify_hash(&c, &bytes), Some(true));
        }
    }
    ...
    async fn test_resolver_seeking_chunked() {
...

        {
            ...
            if let OutContent::Unixfs(node) = ipld_readme.content {
                ...
                assert!(content.starts_with(" beetle")); // without seeking '# beetle'
    ...
    async fn test_unixfs_split_file_regular() {
...

        {
            ...
            if let OutContent::Unixfs(node) = ipld_readme.content {
                ...
                assert!(content.starts_with("# beetle"));
                assert!(content.ends_with("</sub>\n\n"));
            } else {
                panic!("invalid result: {ipld_readme:?}");
            }
    ...
    async fn test_unixfs_hamt_dir() {
        ...
        let car_reader = beetle_car::CarReader::new(reader).await.unwrap();
