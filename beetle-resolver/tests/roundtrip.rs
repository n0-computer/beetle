...
use beetle_metrics::resolver::OutMetrics;
use beetle_unixfs::{
...
use beetle_resolver::resolver::{read_to_vec, stream_to_resolver, Out, Resolver};
...
async fn build_testdir(
    stream: impl Stream<Item = Result<(beetle_resolver::resolver::Path, Out)>>,
...
async fn dir_roundtrip_test(dir: TestDir, hamt: bool) -> Result<bool> {
    ...
    let stream =
        resolver.resolve_recursive_with_paths(beetle_resolver::resolver::Path::from_cid(root));
...
async fn file_roundtrip_test(
    ...
    let (root, resolver) = stream_to_resolver(stream).await?;
    let out = resolver
        .resolve(beetle_resolver::resolver::Path::from_cid(root))
        .await?;
