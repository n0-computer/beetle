...
use beetle_p2p::{config, Config, Keychain, MemoryStorage, NetworkEvent, Node};
use beetle_resolver::resolver::Resolver;
use beetle_rpc_client::Client;
use beetle_rpc_types::Addr;
use beetle_unixfs::{
    content_loader::{ContentLoader, ContextId, LoaderContext, BEETLE_STORE},
    ...
    async fn load_cid(&self, cid: &Cid, _ctx: &LoaderContext) -> Result<LoadedCid> {
        ...
        match self.client.try_store()?.get(cid).await {
            Ok(Some(data)) => {
                return Ok(LoadedCid {
                    data,
                    source: Source::Store(BEETLE_STORE),
                });
            }
            Ok(None) => {}
            Err(err) => {
...
impl P2pNode {
    pub async fn new(port: u16, db_path: &Path) -> Result<(Self, Receiver<NetworkEvent>)> {
        ...
        let rpc_store_client_config = beetle_rpc_client::Config {
            p2p_addr: Some(rpc_p2p_addr_client.clone()),
            store_addr: Some(rpc_store_addr_client.clone()),
            gateway_addr: None,
            channels: Some(1),
        };
        let rpc_p2p_client_config = beetle_rpc_client::Config {
            p2p_addr: Some(rpc_p2p_addr_client.clone()),
            store_addr: Some(rpc_store_addr_client.clone()),
            gateway_addr: None,
            channels: Some(1),
        ...
        let resolver = beetle_resolver::resolver::Resolver::new(loader);

        let store_config = beetle_store::Config {
            path: db_path.to_path_buf(),
            rpc_client: rpc_store_client_config,
        };

        let store = if store_config.path.exists() {
            beetle_store::Store::open(store_config).await?
        } else {
            beetle_store::Store::create(store_config).await?
        };
        ...
        let store_task = tokio::spawn(async move {
            beetle_store::rpc::new(rpc_store_addr_server, store)
                .await
                .unwrap()
        });
