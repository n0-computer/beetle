...
use beetle_util::{beetle_data_path, insert_into_config_map};
...
/// For example, `BEETLE_STORE_PATH=/path/to/config` would set the value of the `Config.path` field
pub const ENV_PREFIX: &str = "BEETLE_STORE";

/// the path to data directory. If arg_path is `None`, the default beetle_data_path()/store is used
/// beetle_data_path() returns an operating system-specific directory
pub fn config_data_path(arg_path: Option<PathBuf>) -> Result<PathBuf> {
    match arg_path {
        Some(p) => Ok(p),
        None => beetle_data_path("store").map_err(|e| anyhow!("{}", e)),
    }
}
...
/// integrated into another binary like in beetle-one, beetle-share or beetle-embed.
